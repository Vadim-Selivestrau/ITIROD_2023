...
function save_task_event() {
  document.querySelectorAll(".save_task").forEach((item) =>
    item.addEventListener("click", function (event) {
      event.preventDefault();
      console.log("save task button pressed");

      var taskName = event.target.parentElement.parentElement.querySelector(
        ".new_task input[placeholder='task name']"
      ).value;
      var taskDescription =
        event.target.parentElement.parentElement.querySelector(
          ".new_task textarea[placeholder='task description']"
        ).value;
      var taskPriority = event.target.parentElement.parentElement.querySelector(
        ".new_task select[name='set_priority']"
      ).value;
      var taskDeadline = event.target.parentElement.parentElement.querySelector(
        ".new_task input[type='date']"
      ).value;

      const sectionCreate = parseInt(
        item.closest("section").id.replace("s", "")
      );
      const taskCreate = parseInt(item.closest("li").id.replace("t", ""));
      board.projects[currentProjectID].sections[sectionCreate].tasks[
        taskCreate
      ] = {
        data: taskDeadline,
...
export function create_task_event() {
  document.querySelectorAll(".create_task").forEach((item) =>
    item.addEventListener("click", function (event) {
...
function cancel_edit_task_event() {
  document.querySelectorAll(".cancel_task").forEach((item) =>
    item.addEventListener("click", function (event) {
      event.preventDefault(); // Prevent form submission
...
export function done_task_event() {
  document
    .querySelectorAll(".done_task, .cross_task")
    .forEach((item) => item.addEventListener("click", addEvent));
}
export function trash_task_event() {
  document
    .querySelectorAll(".trash_task")
    .forEach((item) => item.addEventListener("click", trashEvent));
}

export function edit_task_event() {
  document
    .querySelectorAll(".edit_task")
    .forEach((item) => item.addEventListener("click", editEvent));
}
...
function trashEvent(event) {
  ...
  board.projects[currentProjectID].sections[sectionCreate].tasks.splice(
    taskCreate,
    1
  );
  setBoardData(board, userData);
...
export function trash_section_event() {
  ...
  document
    ...
    .forEach((item) => item.addEventListener("click", trashSectionEvent));
}
//------------------------------------------

export function create_section_event() {
  document
    .querySelector("#new_section")
...
export function render_topic_event() {
  document
    .querySelectorAll(".item_collection")
    .forEach((item) => item.addEventListener("click", rendetTopic));
}
